...
struct node1 {
    int nst[21];  // Increased size to 21 for indexing 1..20
};
...
static char c; // <-- move c here as a char

//static int set[21], nostate, noalpha, s, notransition, nofinal, start, finalstate[21], c, r, buffer[21];
int complete = -1;
char alphabet[21];
static int eclosure[21][21] = {0};
struct node1 hash[21];
struct node *transition[21][21] = {NULL};
...
int main() {
    ...
    for (i = 0; i < notransition; i++) {
        // Added space before %c to skip whitespace
        scanf("%d %c%d", &r, &c, &s);
        insert(r, c, s);
    }
